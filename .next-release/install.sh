...
CLAUDE_VER="$(engine_version claude)"
CODEX_VER="$(engine_version codex)"
AGY_VER="$(engine_version agy)"
...
if [ -n "$CODEX_VER" ]; then
  success "ChatGPT Codex OK ($CODEX_VER)"
elif command -v codex >/dev/null 2>&1; then
  warning "Codex (codex) is on PATH but does not run — check that the installation is intact."
  log "  Check:     codex --version"
  log "  Reinstall: npm i -g @openai/codex"
fi

if [ -n "$AGY_VER" ]; then
  success "Antigravity OK ($AGY_VER)"
elif command -v agy >/dev/null 2>&1; then
  warning "Antigravity (agy) is on PATH but does not run — check that the installation is intact."
  log "  Check: agy --version"
fi

if [ -n "$CLAUDE_VER" ] || [ -n "$CODEX_VER" ] || [ -n "$AGY_VER" ]; then
  :
else
  warning "No usable agent CLI — the dashboard starts, but no task will run."
  log "  HyperTeams drives a CLI you install yourself. Any one of these is enough:"
  log ""
  log "  Claude Code:   curl -fsSL https://claude.ai/install.sh | bash"
  log "                 then run 'claude' once and log in (subscription or API key)."
  log "  ChatGPT Codex: npm i -g @openai/codex"
  log "                 then run 'codex login' once and sign in to ChatGPT."
  log "  Antigravity:   curl -fsSL https://antigravity.google/cli/install.sh | bash"
  log "                 then run 'agy' once and sign in with your Google account."
  log ""
  if may_install "Claude Code (user-local, into ~/.local/bin)" 0 && install_claude; then
    ...
    log "  Once any one of 'claude', 'codex' or 'agy' reports a version you are ready."
  fi
fi
...
CLI_NAME="hyperteams"
[ "$SHIM_OK" = "1" ] || CLI_NAME="node \"$INSTALL_DIR/scripts/cli.mjs\""
DASHBOARD_URL=""
AUTOSTART_ON=0
if [ "${HYPERTEAMS_AUTOSTART:-1}" = "0" ]; then
  log "HYPERTEAMS_AUTOSTART=0 — autostart was left off and nothing was started."
elif [ ! -f .env.local ]; then
  warning "Setup is not finished, so HyperTeams was not started."
elif ! command -v node >/dev/null 2>&1; then
  warning "Node.js was not found, so HyperTeams was not started."
else
  log "Turning on autostart and starting HyperTeams..."
  if DASHBOARD_URL="$(node "$INSTALL_DIR/scripts/cli.mjs" autostart --print-url < /dev/null)"; then
    AUTOSTART_ON=1
  else
    warning "Could not turn on autostart — starting it once in the background instead."
    DASHBOARD_URL="$(node "$INSTALL_DIR/scripts/cli.mjs" start --print-url < /dev/null)" || DASHBOARD_URL=""
  fi
  DASHBOARD_URL="$(printf '%s\n' "$DASHBOARD_URL" | tail -1)"
fi
...
if [ "$SHIM_OK" = "1" ]; then
  if ! command -v hyperteams >/dev/null 2>&1; then
    warning "To use the 'hyperteams' command, open a new terminal or run this first:"
    log "  exec \$SHELL -l"
    log ""
  fi
fi
if [ "$AUTOSTART_ON" = "1" ]; then
  log "It starts automatically when this computer starts. To turn that off:"
  log "  $CLI_NAME autostart off"
else
  log "To start it automatically when this computer starts:"
  log "  $CLI_NAME autostart"
fi
log ""
log "To reconfigure:"
if [ "$SHIM_OK" = "1" ]; then
...
if [ -n "$DASHBOARD_URL" ]; then
  log ""
  success "HyperTeams is running. Open the dashboard:"
  log "  $DASHBOARD_URL"
fi
