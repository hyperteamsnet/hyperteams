...
pty_posix_spawn(char** argv, char** env,
                ...
                pid_t* pid,
                int* err) {
  // Locals declared up front so the shared `cleanup:` exit can run on every
  // path without a goto crossing an initialization that is in scope at it.
  int low_fds[3];
  size_t count = 0;
  ...
  *err = 0;
  ...
  // How many reservation fds were actually opened: indices 0..count inclusive
  // when the loop broke, or all three when it ran to completion (count == 3).
  low_fd_count = (count < 3) ? count + 1 : 3;
...
cleanup:
  // The parent only keeps the master fd; the slave was opened solely to seed the
  // child's stdio through posix_spawn's file actions. The original code never
  // closed the parent's copy of it — one pty slave fd leaked on every spawn.
  if (slave != -1)
    close(slave);
  // Release the low-fd reservations. The original loop started at
  // low_fds[count] and stopped at > 0, so it never closed low_fds[0] (and on
  // the early-return error paths it never ran at all) — another leaked
  // /dev/ptmx fd per spawn. Together these leaks marched the process toward its
  // open-file limit until every posix_openpt() above failed with EMFILE, which
  // — returned without setting *err — surfaced as the generic
  // "posix_spawnp failed.".
  for (size_t i = 0; i < low_fd_count; i++)
    close(low_fds[i]);
  // A failed spawn must not strand the master fd either.
  if (*err != 0 && *master != -1) {
    close(*master);
    *master = -1;
  }
