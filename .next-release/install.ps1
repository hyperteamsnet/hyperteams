...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-ErrExit "Node.js is required (it runs the app - unlike before, it is no longer bundled in the artifact). Install: https://nodejs.org/ (then check 'node --version'). The required major version is reported at boot."
}
...
if (-not (Get-Command cloudflared -ErrorAction SilentlyContinue)) {
    Write-Warn "cloudflared not found - only needed if you want remote access (tunnel)."
    Write-Log  "  Install: https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/downloads/"
}
