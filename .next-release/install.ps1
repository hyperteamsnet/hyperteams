...
function Get-CodexEntry {
    $sources = @(Get-Command codex -All -ErrorAction SilentlyContinue |
                 Where-Object { $_.Source } | ForEach-Object { $_.Source })
    foreach ($src in $sources) {
        if ([IO.Path]::GetExtension($src) -in @(".exe", ".com")) {
            return @{ Path = $src; Usable = $true }
        }
        $dir = Split-Path $src -Parent
        $abs = Join-Path $dir "node_modules\@openai\codex\bin\codex.js"
        if (Test-Path $abs) { return @{ Path = $abs; Usable = $true } }
    }
    if ($sources.Count -gt 0) { return @{ Path = ($sources -join ", "); Usable = $false } }
    return $null
...
function Get-AgyEntry {
    $cmd = Get-Command agy -ErrorAction SilentlyContinue | Select-Object -First 1
    if (-not $cmd -or -not $cmd.Source) { return $null }
    try {
        $ver = (& $cmd.Source --version 2>$null | Select-Object -First 1)
        if ($LASTEXITCODE -eq 0 -and $ver) { return @{ Path = $cmd.Source; Usable = $true; Version = "$ver".Trim() } }
    } catch { }
    return @{ Path = $cmd.Source; Usable = $false }
}
...
if ($codex -and $codex.Usable) {
    Write-Ok "ChatGPT Codex OK"
} elseif ($codex) {
    Write-Warn "Codex is on PATH, but not in a form this dashboard can execute."
    Write-Log  "  Found: $($codex.Path)"
    Write-Log  "  Reinstall: npm i -g @openai/codex"
}

if ($agy -and $agy.Usable) {
    Write-Ok "Antigravity OK ($($agy.Version))"
} elseif ($agy) {
    Write-Warn "Antigravity (agy) is on PATH but does not run - check that the installation is intact."
    Write-Log  "  Found: $($agy.Path)"
    Write-Log  "  Check: agy --version"
}

if (-not (($claude -and $claude.Usable) -or ($codex -and $codex.Usable) -or ($agy -and $agy.Usable))) {
    Write-Warn "No usable agent CLI - the dashboard starts, but no task will run."
    Write-Log  "  HyperTeams drives a CLI you install yourself. Any one of these is enough:"
    Write-Log  ""
    Write-Log  "  Claude Code:   irm https://claude.ai/install.ps1 | iex"
    Write-Log  "                 (with winget: winget install Anthropic.ClaudeCode)"
    Write-Log  "                 then run 'claude' once and log in (subscription or API key)."
    Write-Log  "  ChatGPT Codex: npm i -g @openai/codex"
    Write-Log  "                 then run 'codex login' once and sign in to ChatGPT."
    Write-Log  "  Antigravity:   irm https://antigravity.google/cli/install.ps1 | iex"
    Write-Log  "                 then run 'agy' once and sign in with your Google account."
    Write-Log  ""
    if ((Confirm-AutoInstall "Claude Code (user-local install)") -and (Install-Claude)) {
        Write-Ok "Claude Code installed"
        $script:ClaudeJustInstalled = $true
    } else {
        Write-Log "  Once any one of 'claude', 'codex' or 'agy' reports a version you are ready."
    }
}
...
$CliName = if ($ShimOk) { "hyperteams" } else { "node `"$InstallDir\scripts\cli.mjs`"" }
$DashboardUrl = ""
$AutostartOn = $false
if ($env:HYPERTEAMS_AUTOSTART -eq "0") {
    Write-Log "HYPERTEAMS_AUTOSTART=0 - autostart was left off and nothing was started."
} elseif (-not (Test-Path "$InstallDir\.env.local")) {
    Write-Warn "Setup is not finished, so HyperTeams was not started."
} elseif (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Warn "Node.js was not found, so HyperTeams was not started."
} else {
    Write-Log "Turning on autostart and starting HyperTeams..."
    $prevEap = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
        $DashboardUrl = (& node "$InstallDir\scripts\cli.mjs" autostart --print-url | Select-Object -Last 1)
        if ($LASTEXITCODE -eq 0) {
            $AutostartOn = $true
        } else {
            Write-Warn "Could not turn on autostart - starting it once in the background instead."
            $DashboardUrl = (& node "$InstallDir\scripts\cli.mjs" start --print-url | Select-Object -Last 1)
            if ($LASTEXITCODE -ne 0) { $DashboardUrl = "" }
        }
    } catch {
        Write-Warn "Could not start HyperTeams: $_"
        $DashboardUrl = ""
    }
    $ErrorActionPreference = $prevEap
    if (-not $DashboardUrl) { $DashboardUrl = "" }
}
...
if ($ShimOk) {
    Write-Log "(In other terminal windows, open a new one before using 'hyperteams'.)"
    Write-Log ""
}
if ($AutostartOn) {
    Write-Log "It starts automatically when you sign in to Windows. To turn that off:"
    Write-Log "  $CliName autostart off"
} else {
    Write-Log "To start it automatically when you sign in to Windows:"
    Write-Log "  $CliName autostart"
}
Write-Log ""
...
if ($DashboardUrl) {
    Write-Log ""
    Write-Ok "HyperTeams is running. Open the dashboard:"
    Write-Log "  $DashboardUrl"
}
