...
`),process.exit(1)},x=process.argv.slice(2),y=new Set(x.filter(t=>t.startsWith("--"))),I=x.filter(t=>!t.startsWith("--")),Ot=I[0]&&!I[0].startsWith("-")?I.shift():"start";function Dt(t,e){let r=x.indexOf(`--${t}`);return r>=0&&x[r+1]&&!x[r+1].startsWith("--")?x[r+1]:e}var m=Dt("command-name","hyperteams"),ie=y.has("--yes")||y.has("-y");function pt(t,e,r={}){let s=process.listeners("SIGINT");process.on("SIGINT",()=>{});try{let o=S(t,e,{stdio:"inherit",...r});return o.error&&p(`Failed to run ${t}: ${o.error.message}`),o.status??1}finally{process.removeAllListeners("SIGINT");for(let o of s)process.on("SIGINT",o)}}function Ct(){a.existsSync(i.join(l,"server.js"))||p("server.js is missing \u2014 the release package is corrupted."),a.existsSync(i.join(l,".env.local"))||p(`.env.local is missing. Configure it with:
    ${m} setup`)}function ae(){n("Checking the setup..."),Ct(),d("Setup looks good"),process.stderr.write(`
`),n("Starting HyperTeams..."),n("The dashboard address is printed in the [supervise] lines below."),n("Stop: Ctrl-C"),process.stderr.write(`
`)}function Nt(t){if(y.has("--background")||y.has("--bg"))return nt();if(g){let r=i.join(l,"scripts","supervise.mjs");return a.existsSync(r)||p(`Start script not found: ${r}`),ae(),pt(process.execPath,[r,...t])}let e=i.join(l,"scripts","start.sh");return a.existsSync(e)||p(`Start script not found: ${e}`),pt("bash",[e,...t])}var v=i.join(l,"logs","server.log");function et(){let t=Number(process.env.PORT);if(t)return t;try{let e=a.readFileSync(i.join(l,".env.local"),"utf8").match(/^\s*PORT\s*=\s*["']?(\d+)/m);if(e)return Number(e[1])}catch{}return 27777}function D(t){return new Promise(e=>{let r=ee.connect({host:"127.0.0.1",port:t}),s=o=>{r.destroy(),e(o)};r.setTimeout(500),r.on("connect",()=>s(!0)),r.on("timeout",()=>s(!1)),r.on("error",()=>s(!1))})}function K(t){try{return a.readFileSync(v,"utf8").trimEnd().split(`
`).slice(-t)}catch{return[]}}async function nt(){n("Checking the setup..."),Ct();let t=et();!y.has("--force")&&await D(t)&&p(`Something is already listening on port ${t} \u2014 it is probably already running.
...
`),d(`Running in the background (pid ${s.pid})`),c?n(`Dashboard:  ${c}`):h("It is still starting \u2014 the address will appear in the log."),n(`Log:        ${v}`),n(`Stop:       ${W()?.commandName??m} stop`),0}function ce(t){let e=i.join(l,"scripts","setup.mjs");return a.existsSync(e)||p(`Setup script not found: ${e}`),pt(process.execPath,[e,...t])}var B="net.hyperteams.app",j="hyperteams.service";function le(){let t=process.env.APPDATA||i.join(w,"AppData","Roaming");return i.join(t,"Microsoft","Windows","Start Menu","Programs","Startup")}function rt(){return process.platform==="darwin"?{kind:"launchd",file:i.join(w,"Library","LaunchAgents",`${B}.plist`)}:g?{kind:"startup",file:i.join(le(),"HyperTeams.lnk"),helper:i.join(l,"bin","autostart.vbs")}:{kind:"systemd",file:i.join(w,".config","systemd","user",j)}}function ue(){try{return a.existsSync(rt().file)}catch{return!1}}function Ut(){let t=[i.dirname(process.execPath),...(process.env.PATH??"").split(i.delimiter)];g||t.push("/opt/homebrew/bin","/usr/local/bin","/usr/bin","/bin","/usr/sbin","/sbin");let e=new Set;return t.filter(r=>r&&!e.has(r)&&e.add(r)).join(i.delimiter)}function gt(){let t=a.existsSync(i.join(l,"server.js")),e=i.join(l,"scripts",t?"start.sh":"start-all.sh");return a.existsSync(e)||p(`Start script not found: ${e}`),a.existsSync(i.join(l,".env.local"))||p(`.env.local is missing. Configure it with:
    ${m} setup`),e}function de(){if(process.platform!=="darwin")return null;for(let t of["Documents","Desktop","Downloads"]){let e=i.join(w,t);if(l===e||l.startsWith(e+i.sep))return e}return null}function _(t){let e=S("launchctl",t,{encoding:"utf8"});return{status:e.error?1:e.status??1,out:`${e.stdout??""}${e.stderr??""}`.trim()}}function X(){return`gui/${process.getuid?.()??0}`}function R(t){let e=S("systemctl",["--user",...t],{encoding:"utf8"});return{status:e.error?1:e.status??1,out:`${e.stdout??""}${e.stderr??""}`.trim()}}function U(t){return String(t).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function pe(){return`<?xml version="1.0" encoding="UTF-8"?>
...
<dict>
  ...
  <key>WorkingDirectory</key><string>${U(l)}</string>
  <key>EnvironmentVariables</key>
  <dict>
    <key>PATH</key><string>${U(Ut())}</string>
    <key>HYPERTEAMS_BACKGROUND</key><string>1</string>
  </dict>
  <key>RunAtLoad</key><true/>
  <key>StandardOutPath</key><string>${U(v)}</string>
  <key>StandardErrorPath</key><string>${U(v)}</string>
</dict>
</plist>
...
`)}async function me(t,e=60){for(let r=0;r<e*2;r++)if(await A(500),await D(t))return!0;return!1}function Pt(){let t=(I[0]??"").toLowerCase();return y.has("--status")||t==="status"?ye():y.has("--off")||t==="off"?_t():(t&&t!=="on"&&p(`Unknown option: ${t}
  Usage: ${m} autostart [on|off|status]`),ge())}async function ge(){n(`Install directory: ${l}`),n("Checking the setup...");let t=gt();d(g?`Setup looks good (it will run: ${m} start --background)`:`Setup looks good (it will run: bash ${t})`);let e=de();e&&(n(""),h(`This installation lives in a folder macOS protects: ${e}`),n("  Unless Full Disk Access is granted, a login item cannot read files there \u2014"),n('  every login would fail with "Operation not permitted", even though starting'),n("  it by hand works. Two ways out:"),n(`    \xB7 Move the installation out of ${i.basename(e)}/ (e.g. to ~/hyperteams), or`),n("    \xB7 System Settings \u25B8 Privacy & Security \u25B8 Full Disk Access \u25B8 add /bin/bash"),n(""));let r=rt(),s=et(),o=await D(s);try{let c=r.kind==="startup"?l:a.readFileSync(r.file,"utf8");if(!c.includes(l)){h("Replacing a login item that pointed at another installation:");for(let u of c.match(/[^\s<>"]*(?:start-all\.sh|start\.sh|cli\.mjs)/g)??[])n(`    ${u}`);n("  Only one installation can start automatically \u2014 they would fight over the port.")}}catch{}if(a.mkdirSync(i.dirname(r.file),{recursive:!0}),a.mkdirSync(i.dirname(v),{recursive:!0}),r.kind==="launchd"){if(a.writeFileSync(r.file,pe()),d(`Login item written: ${r.file}`),_(["bootout",`${X()}/${B}`]),!o){let c=_(["bootstrap",X(),r.file]),u=c.status===0?null:_(["load","-w",r.file]);if(c.status!==0&&u.status!==0){h("It is registered, but launchd would not start it right now:");for(let f of(c.out||u.out).split(`
...
`);r.kind!=="systemd"&&n(`  The full log is at: ${v}`)}return n(""),r.kind==="launchd"?(n("macOS lists it in System Settings \u25B8 General \u25B8 Login Items (Allow in the Background)."),n("It starts when you log in \u2014 turn on automatic login if this machine must come up on its own.")):r.kind==="systemd"?n(`Log:        journalctl --user -u ${j} -f`):n("It starts when you sign in to Windows."),n(`Turn it off: ${m} autostart off`),0}function _t(t={}){let e=t.quiet===!0,r=rt(),s=a.existsSync(r.file);r.kind==="launchd"?_(["bootout",`${X()}/${B}`]):r.kind==="systemd"&&s&&R(["disable",j]);try{a.rmSync(r.file,{force:!0}),r.helper&&a.rmSync(r.helper,{force:!0})}catch(o){return e||p(`Could not remove ${r.file}: ${o.message}`),0}return r.kind==="systemd"&&s&&R(["daemon-reload"]),e?(s&&d(`Removed the login item: ${r.file}`),0):s?(d(`Autostart is off: ${r.file} was removed.`),n("Anything running right now keeps running \u2014 stop it with:"),n(`    ${m} stop`),0):(d("Autostart was not on \u2014 nothing to remove."),0)}async function ye(){let t=rt(),e=a.existsSync(t.file);if(n(`Install directory: ${l}`),n(""),e?d(`Autostart is on: ${t.file}`):h(`Autostart is off (no ${t.file})`),t.kind==="launchd"){let r=_(["print",`${X()}/${B}`]),s=e?"not loaded yet (it loads at login)":"not loaded";n(`  launchd:      ${r.status===0?"loaded":s}`)}else if(t.kind==="systemd"){let r=R(["is-enabled",j]);n(`  systemd:      ${r.out||"unknown"}`)}return n(`  Running now:  ${await D(et())?"yes":"no"}`),n(""),n(e?`Turn it off:  ${m} autostart off`:`Turn it on:   ${m} autostart`),0}function Ht(t){return g?{binDir:i.join(l,"bin"),file:i.join(l,"bin",`${t}.cmd`)}:{binDir:i.join(w,".local","bin"),file:i.join(w,".local","bin",t)}}var ft="hyperteams-shim",$e=i.join(i.dirname(l),"runtime","node");function At(t){return g?["@echo off",`REM ${ft} \u2014 generated by the installer. Do not edit.`,"REM ASCII only: cmd tracks its read position by byte offset (see scripts/start.bat).",'set "HT_NODE=%~dp0..\\..\\runtime\\node"','if exist "%HT_NODE%\\node.exe" set "PATH=%HT_NODE%;%PATH%"',"where node >nul 2>nul","if errorlevel 1 (","    echo [hyperteams] Node.js is required: https://nodejs.org/ 1>&2","    exit /b 1",")",'node "%~dp0..\\scripts\\cli.mjs" %*',""].join(`\r
`):["#!/usr/bin/env bash",`# ${ft} \u2014 generated by the installer. Do not edit.`,`HT_NODE=${JSON.stringify(i.join($e,"bin"))}`,'[ -x "$HT_NODE/node" ] && PATH="$HT_NODE:$PATH"','command -v node >/dev/null 2>&1 || { echo "[hyperteams] Node.js is required: https://nodejs.org/" >&2; exit 1; }',`exec node ${JSON.stringify(i.join(l,"scripts","cli.mjs"))} "$@"`,""].join(`
`)}function W(){try{return JSON.parse(a.readFileSync(mt,"utf8"))}catch{return null}}function be(t){a.mkdirSync(P,{recursive:!0,mode:448}),a.writeFileSync(mt,JSON.stringify(t,null,2))}function Lt(t={}){let e=t.quiet===!0,r=t.name??m,{binDir:s,file:o}=Ht(r);if(a.existsSync(o)){let f=a.readFileSync(o,"utf8"),b=f.includes(ft);if(!b&&!y.has("--force")){if(e)return h(`Left the existing '${r}' alone \u2014 it is not ours: ${o}`),0;p(`A different '${r}' already exists: ${o}
  Use --force to overwrite, or --command-name <name> to use another name`)}if(e&&b&&f===At(r))return 0}else{let f=we(r);f&&!y.has("--force")&&(h(`'${r}' already exists on PATH: ${f}`),h("  That one may take precedence. Use --command-name <name> for another name"))}a.mkdirSync(s,{recursive:!0}),a.writeFileSync(o,At(r)),g||a.chmodSync(o,493),d(e?`Command refreshed: ${o}`:`Command installed: ${o}`);let c=W(),u=c&&c.binDir&&c.binDir!==s?[c.binDir]:[];return(!e||!c?.binDir||c.binDir!==s)&&(g?Mt(s,u):ve(s)),be({binDir:s,file:o,root:l,commandName:r}),e||process.stdout.write(s+`
`),0}function we(t){let e=g?";":":",r=g?(process.env.PATHEXT||".COM;.EXE;.BAT;.CMD").split(";"):[""];for(let s of(process.env.PATH??"").split(e))if(s)for(let o of r){let c=i.join(s,t+o);try{if(a.statSync(c).isFile())return c}catch{}}return null}function Se(){let t=i.basename(process.env.SHELL||"");return t==="fish"?{file:i.join(w,".config","fish","config.fish"),fish:!0}:t==="zsh"?{file:i.join(w,".zshrc"),fish:!1}:t==="bash"?{file:i.join(w,process.platform==="darwin"?".bash_profile":".bashrc"),fish:!1}:{file:i.join(w,".profile"),fish:!1}}function ke(){return[i.join(w,".zshrc"),i.join(w,".bashrc"),i.join(w,".bash_profile"),i.join(w,".profile"),i.join(w,".config","fish","config.fish")]}function ve(t){let{file:e,fish:r}=Se(),s="";try{s=a.readFileSync(e,"utf8")}catch{}let o=s.includes(J);if(o&&s.includes(t)){d(`PATH entry is already in ${e}`);return}if(!o&&(process.env.PATH??"").split(":").includes(t)){d(`${t} is already on PATH`);return}o&&Ft();let c=r?`fish_add_path -g ${JSON.stringify(t)}`:`export PATH=${JSON.stringify(t)}:"$PATH"`,u=`
${J}
${c}
${Rt}
`;a.mkdirSync(i.dirname(e),{recursive:!0}),a.appendFileSync(e,u),d(`Added the PATH entry: ${e}`)}function Ft(){for(let t of ke()){let e;try{e=a.readFileSync(t,"utf8")}catch{continue}if(!e.includes(J))continue;let r=[],s=!1;for(let o of e.split(`
...
`,r=i.join(tt.tmpdir(),`hyperteams-rm-${process.pid}.ps1`);a.writeFileSync(r,e,"utf8"),dt("powershell.exe",["-NoProfile","-ExecutionPolicy","Bypass","-WindowStyle","Hidden","-File",r],{detached:!0,stdio:"ignore"}).unref()}var ut=process.env.DIST_REPO_URL||"https://github.com/hyperteamsnet/hyperteams.git",H="origin",xe=["data.db","data.db-wal","data.db-shm",".env.local","previews.map","dashboards.map","cloudflared"],jt=[".env.local","previews.map","dashboards.map","cloudflared"],A=t=>new Promise(e=>setTimeout(e,t));function yt(){let t=a.existsSync(i.join(l,"server.js")),e=a.existsSync(i.join(l,".git"));return t?e?"artifact":"artifact-detached":e?"source":"unknown"}var $t={...process.env,GIT_TERMINAL_PROMPT:"0"};function k(t){let e=S("git",["-C",l,...t],{encoding:"utf8",env:$t});return{status:e.error?1:e.status??1,out:(e.stdout??"").trim(),err:(e.stderr??"").trim()}}function Re(t){let e=S("git",["-C",l,...t],{stdio:["ignore","inherit","inherit"],env:$t});return e.error?1:e.status??1}function Oe(){let t=Dt("branch",null);if(t)return t;let e=k(["symbolic-ref","--quiet","--short","HEAD"]);return e.status===0&&e.out?e.out:`dist-${process.platform}-${process.arch}`}function De(){let t=k(["remote","get-url",H]);if(t.status===0&&t.out)return t.out;let e=k(["remote","add",H,ut]);return e.status!==0&&p(`Could not set the download source: ${e.err}`),h(`The download source was missing \u2014 restored it: ${ut}`),ut}function Jt(){return g?null:z().length}function Kt(){let t=process.env.DB_PATH||i.join(l,"data.db");if(!a.existsSync(t))return null;let e;try{e=ht(import.meta.url).resolve("better-sqlite3")}catch{return null}let r=`
...
`,o=(S(process.execPath,["-e",r,e,t],{encoding:"utf8",timeout:15e3,stdio:["ignore","pipe","ignore"]}).stdout??"").trim(),c=Number(o);return o!==""&&Number.isFinite(c)?c:null}function Ce(t){let e=k(["show",`${t}:.node-requirement.json`]);if(e.status!==0)return null;let r;try{r=JSON.parse(e.out)}catch{return null}let s=Array.isArray(r.sqliteAbis)?r.sqliteAbis:[];if(s.length===0)return null;let o=String(process.versions.modules);return s.some(c=>String(c.abi)===o)?null:`The new version does not support this Node.
  ...
  It was required to install too, so it may have been removed since: https://git-scm.com/downloads`);let s=Oe(),o=De(),c=M();n(`Install directory: ${l}`),n(`Current version:   v${c}`),n(`Channel:           ${s}`),n("Checking for a new version...");let u=k(["rev-parse","HEAD"]).out,f=k(["ls-remote",H,`refs/heads/${s}`]);f.status!==0&&p(`Could not reach the release repository \u2014 check your internet connection.
  Source: ${o}
  ${f.err.split(`
...
`),d(`This is the latest version (v${c}) \u2014 nothing to do.`),t||n("Re-download it anyway with --force."),0;let E=t?0:Jt(),q=t?0:Kt();if(!t&&(n(""),n($?"Re-downloading the current version:":`Update found: ${b.slice(0,7)}`),n("  \xB7 downloads the latest build for this system"),n("  \xB7 replaces the program files only \u2014 your DB, settings and tunnel credentials stay"),E===null?n("  \xB7 stops the app if it is running (it must be restarted afterwards)"):E>0&&n(`  \xB7 stops the app (${E} process${E===1?"":"es"} running now)`),q&&(n(""),h(`${q} job${q===1?" is":"s are"} running right now \u2014 restarting interrupts ${q===1?"it":"them"}.`)),n(""),!await O("Upgrade now?")))return n("Cancelled \u2014 nothing was changed."),1;!t&&E!==0&&(n("Stopping the app..."),ot(),it(),await A(g?2e3:300),n("")),n("Downloading the new version...");let C=`refs/remotes/${H}/${s}`;Re(["fetch","--depth","1",H,`+refs/heads/${s}:${C}`])!==0&&p(`Download failed \u2014 nothing was changed.
  Check your internet connection and run '${m} upgrade' again.${t?"":`
  The app was stopped, so start it with '${m}' if you want to keep using this version.`}`),d("Download complete");let St=k(["ls-tree","-r","--name-only",C,"--",...xe]).out;St&&p(`The new build would overwrite your data \u2014 stopping. Nothing was changed.
  Offending paths: ${St.split(`
`).join(", ")}
  Please report this; do not upgrade until it is fixed.`);let G=Ce(C);if(G&&!y.has("--force")&&p(`${G}
  Nothing was changed \u2014 the current version is still installed.`),t)return process.stdout.write(`prepared
`),d(`Downloaded and verified: ${b.slice(0,7)} \u2014 not applied yet.`),n(`Apply it with:  ${m} upgrade`),0;let kt=Ne();n("Applying..."),k(["symbolic-ref","HEAD",`refs/heads/${s}`]);let T=Q(C);for(let N=1;g&&N<=3&&T.status!==0;N++){let lt=T.locked.filter(Xt);if(lt.length){let Tt=lt.length;n(`${Tt} file${Tt===1?" was":"s were"} locked \u2014 set aside, retrying...`)}else n("Some files were in use \u2014 waiting and retrying...");await A(1e3*N),T=Q(C)}if(T.status!==0){process.stderr.write(T.text.trimEnd()+`
`);let N=T.locked.length?`
  Still in use: ${T.locked.join(", ")}`:"";p(`Could not replace the program files.${N}
...
`,o=S(process.execPath,["-e",s,r,e,JSON.stringify(t)],{encoding:"utf8",timeout:15e3,stdio:["ignore","pipe","ignore"]});if(o.status!==0||!(o.stdout??"").trim())return null;try{return JSON.parse(o.stdout)}catch{return null}}function L(t,e){let r=V();if(!a.existsSync(r))return!1;let s;try{s=ht(import.meta.url).resolve("better-sqlite3")}catch{return!1}let o=`
const Database = require(process.argv[1]);
...
`).slice(-1)[0]||"could not write to the database"),!1):!0}function F(t,e){if(!t)return e;try{return JSON.parse(t)??e}catch{return e}}var He="hyperteams-rollback-",Le=".backup-autoupdate-";function Zt(){let t=k(["tag","--list",`${He}*`]);return t.status===0&&t.out?t.out.split(`
`).map(e=>e.trim()).filter(Boolean).sort():[]}function Qt(){let t=i.dirname(l),e=i.basename(l)+Le;try{return a.readdirSync(t).filter(r=>r.startsWith(e)).map(r=>i.join(t,r,"data.db")).filter(r=>a.existsSync(r)).sort()}catch{return[]}}function Y(t){let e=Number(t);return Number.isFinite(e)&&e>0?new Date(e*1e3).toISOString():"\u2014"}function Fe(t){let e=F(t?.["autoUpdate:rollbackPending"],null)??F(t?.["autoUpdate:attempt"],null);if(e?.rollbackRef||e?.fromSha)return{ref:e.rollbackRef??null,fallbackRef:e.fromSha??null,dbBackup:e.dbBackup??null,fromVersion:e.fromVersion??null,toVersion:e.toVersion??null,guessed:!1};let r=Zt(),s=Qt();return!r.length&&!s.length?null:{ref:r.length?r[r.length-1]:null,fallbackRef:null,dbBackup:s.length?s[s.length-1]:null,fromVersion:null,toVersion:null,guessed:!0}}function Me(...t){for(let e of t)if(e&&k(["rev-parse","--verify","--quiet",`${e}^{commit}`]).status===0)return e;return null}function Be(){let t=bt(zt);if(n(`Install directory: ${l}`),n(`Current version:   v${M()}`),n(`Installation kind: ${yt()}`),n(""),!t)h(`Could not read ${V()} \u2014 showing only what is on disk.`);else{let s=$=>t[$]===void 0?!0:t[$]==="1",o=s("autoUpdate:enabled");n("Automatic updates"),n(`  Checking:        ${o?"on":"off"}`),n(`  Interval:        every ${t["autoUpdate:intervalHours"]??"6"}h`),n(`  Apply up to:     ${t["autoUpdate:scope"]??"patch"}`);let c=s("autoUpdate:checkOnBoot");n(`  Check on start:  ${c?"yes":"no"}`),n(`  Rollback waits:  ${t["autoUpdate:rollbackWaitMinutes"]??"30"}m for running work`),n(`  Last checked:    ${Y(t["autoUpdate:lastCheckAt"])}`),n(`  Latest seen:     v${t["autoUpdate:detectedVersion"]||"?"}`),t["autoUpdate:skipVersion"]&&n(`  Skipping:        v${t["autoUpdate:skipVersion"]} (it was rolled back)`),n("");let u=F(t["autoUpdate:lastResult"],null);u&&(n("Last attempt"),n(`  Outcome:         ${u.outcome}`),n(`  When:            ${Y(u.at)}`),n(`  Versions:        v${u.fromVersion} \u2192 v${u.toVersion??"?"}`),u.detail&&n(`  Detail:          ${u.detail}`),n(""));let f=F(t["autoUpdate:attempt"],null);f&&(h("An update is in flight (the app left a record and has not reported back yet)."),n(`  Started:         ${Y(f.startedAt)}`),n(""));let b=F(t["autoUpdate:rollbackPending"],null);b&&(h("A rollback is queued \u2014 it is waiting for running work to finish."),n(`  Waiting since:   ${Y(t["autoUpdate:rollbackPendingAt"])}`),n(`  Would restore:   ${b.rollbackRef} and ${b.dbBackup}`),n("")),t["autoUpdate:halted"]&&(h(`Automatic updates are halted: ${t["autoUpdate:halted"]}`),n(`  Clear it with:   ${m} auto-update --resume`),n(""))}let e=Zt(),r=Qt();return n("Recovery material on disk"),n(`  Rollback tags:   ${e.length?e.join(", "):"(none)"}`),n(`  DB snapshots:    ${r.length?r.join(`
                   `):"(none)"}`),n(""),(e.length||r.length)&&n(`Roll back with:  ${m} rollback`),0}function We(){if(!y.has("--resume"))return n(`Usage: ${m} auto-update --resume`),n(""),n(`Shows the current state with:  ${m} upgrade-status`),n("Everything else is configured from the dashboard (Settings \u2192 Update)."),1;let t=bt(["autoUpdate:halted","autoUpdate:rollbackPending"]);return t==null&&p(`Could not open ${V()} \u2014 is this the installation directory?`),t["autoUpdate:halted"]?(n(`Halted because: ${t["autoUpdate:halted"]}`),L("autoUpdate:halted","")||p("Could not clear it. If the app is running, use Settings \u2192 Update \u2192 Resume instead."),t["autoUpdate:rollbackPending"]&&L("autoUpdate:rollbackPendingAt",String(Math.floor(Date.now()/1e3))),d("Cleared. Automatic updates will try again at the next check."),0):(d("Automatic updates are not halted \u2014 nothing to do."),0)}async function Ve(){let t=yt();(t==="source"||t==="unknown")&&p(`This is not a packaged installation, so there is nothing to roll back.
  ...
  To reinstall the previous version, run the install command again.`);let o=Me(s.ref,s.fallbackRef);o||p(`The rollback point is gone from this repository${s.ref?` (${s.ref})`:""}.
...
Install directory: ${l}
`),0}var qe={start:()=>Nt(I),stop:()=>Gt(),restart:()=>Pe(I),setup:()=>ce(I),upgrade:()=>It(),"upgrade-status":()=>Be(),rollback:()=>Ve(),"auto-update":()=>We(),update:()=>It(),autostart:()=>Pt(),"auto-start":()=>Pt(),"install-shim":()=>Lt(),uninstall:()=>Ae(),help:()=>wt()};y.has("--version")&&(process.stdout.write(M()+`
`),process.exit(0));(y.has("--help")||y.has("-h"))&&process.exit(wt());var te=qe[Ot];te||(process.stderr.write(`\x1B[91m\u2717 Unknown command: ${Ot}\x1B[0m
