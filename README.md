...
step 2, it checks for git, Node.js and an agent CLI, asks before it fills in
...
cannot do for you is **sign in**.

**You only need one agent CLI** — Claude Code, ChatGPT Codex or Antigravity. The installer
takes them together: if either one runs, it moves on quietly. Only when neither
is usable does it stop and show you both routes, and the one it can install for
you on the spot is Claude Code (that one has a sudo-free user-local installer).
Add the other later and the dashboard finds it by itself.
...
- **ChatGPT Codex** — a second engine that runs on your ChatGPT subscription:
  billed against that plan's usage windows rather than per token.
  ...
  When `codex --version` prints a version, you are set. Pick the engine when you
  create a task; permission modes, model choice, approval cards and question
  cards all work through the same screens as Claude Code. Sub-agents are tracked
  and folded the same way. **Installing and removing** the working folder's MCP /
  skills is still Claude Code only; Codex's show as a read-only list in Tools.

- **Antigravity (agy)** — a third engine, running Gemini-family models. Install
  the CLI and run it once to sign in with your Google account.

  ```bash
  curl -fsSL https://antigravity.google/cli/install.sh | bash   # macOS / Linux
  # Windows (PowerShell): irm https://antigravity.google/cli/install.ps1 | iex
  agy             # run once and sign in
  agy --version   # a version means you are set
  ```

  Fourteen models (Gemini 3.x, Claude, GPT-OSS) and three thinking depths. It
  offers **only two permission modes** — «Fully automatic» and «Auto-accept
  edits» — because agy cannot prompt for permission in headless mode, so a mode
  that asks would simply fail. Pick «Fully automatic» if it needs to run commands.
...
When it finishes, the installer **turns on autostart and starts HyperTeams**, then
prints the address as its last line — usually **`http://localhost:27777`**. Open
it in your browser, enter your password, and that is it. From then on it comes
up whenever you log in to this computer; if you would rather start it yourself,
turn that off with `hyperteams autostart off` (see
[Start it with the computer](#start-it-with-the-computer)). To skip it during an
unattended install, set `HYPERTEAMS_AUTOSTART=0`.

### 3. Start

If it is not running — you stopped it, or turned autostart off — one word starts
it again, from any folder:

```bash
hyperteams
```

The address is printed on screen as it starts. To stop it: `hyperteams stop`.
...
### Run to completion without a person — Autopilot and the Jev judge (v0.2.0)

Pick a level in folder settings › **Automation**.

| Level | What happens |
|---|---|
| **Manual** | As before. A person commits, merges and reviews. |
| **Auto-land** | Each task runs in its own worktree and is committed and pushed when it finishes. |
| **Autopilot** | Runs without approvals or questions, judges the result after every turn and keeps going **until it succeeds**. |

On autopilot, a judge sorts every finished turn into **success · needs review · failed** and picks the next step itself —
it fixes failures at the root, finishes half-done work, makes the choice it was asked to wait on, and when a "check this"
turn found an obvious defect it applies the fix right away. A merge halted by a conflict or failing checks goes back to the
agent too. Only irreversible steps nobody asked for (payments, production deploys, deletions, outbound sends) are prepared in
a reversible form with the last step left for a person — if the instruction explicitly asked for them, they are done.

Choose the judge in global settings › **Judge**.

- **Jev (TypeSafe)** — a model built for judging. About 0.2 s and $0.00014 per judgment. On 5,161 real turns: 96.3% binary
  accuracy and 2.4% false alarms — on par with a human labeller. Needs an API key; the task's instruction and last answer are
  sent to TypeSafe when judging.
- **Keyword rules** — free, nothing leaves this computer, but weaker at telling when a turn is waiting on a person.
...
`hyperteams` **starts in the background.** It prints the address, hands the
prompt back, and keeps running after you close the window. Everything that would
have scrolled past goes to `logs/server.log` in the install folder, with the
previous run kept as `logs/server.log.prev`. Stop it with `hyperteams stop`.

To watch it scroll instead, give it this terminal:

```bash
hyperteams start --foreground
```

Then Ctrl-C is how it ends, and closing the window takes the server with it.

### It comes back on its own when it dies

If the app, the router or the tunnel exits unexpectedly, the supervisor brings
all three back. Repeated failures back off — 2s, 5, 15, 30, 60 — and it keeps
trying; once a start survives a minute, the count returns to zero. What happened
stays in `logs/server.log`, as `restarting everything in 5s (attempt 2)`.

**What a person stopped stays stopped.** `hyperteams stop` and Ctrl-C reach the
supervisor itself, so they never read as a crash, and a login item is taken down
as a service before anything is killed.
...
The installer already does this for you; the command is for turning it back on
later. It registers a login item — launchd on macOS, systemd on Linux, the Startup
folder on Windows. Turn it off with `hyperteams autostart off`, and see how it
stands with `hyperteams autostart status`.

Four things worth knowing.

- **It is resurrected when it dies, not when you stop it.** The OS restarts it
  only on a failed exit (launchd `KeepAlive`, systemd `Restart=on-failure`, 30s
  apart). `hyperteams stop` takes the service down before killing anything, so
  what you stopped stays stopped and comes back at the next login.
...
terminals. **However you started it** — holding a terminal, in the background,
or from the login item — this one line stops it (a login item keeps its
registration and comes back at the next login). If a job is running it says so
and asks first, because a running job does not survive a restart. Start it again
with `hyperteams`.
