 ...
 * for what gets sent and lib/tasks/notify.ts for what it says.
 *
 ...
 * Mirrored in lib/ui/browser/vapid-key.ts, which the page side subscribes with.
 */
  ...
  event.waitUntil(
    (async () => {
      try {
        ...
        // The page re-registers on its next load (components/hooks/usePush.ts).
      }
    })(),
  );
});
